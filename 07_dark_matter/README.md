Introduction

Dark Matter is one of the greatest mysteries in modern astrophysics. 

Although we cannot directly observe it, astronomers have found overwhelming evidence that a large amount of invisible matter exists throughout the universe.

When scientists studied how galaxies rotate, they discovered something surprising.

According to Newton's law of gravity, stars farther from the center of a galaxy should orbit more slowly.

Instead, observations show that stars near the edges of galaxies move almost as fast as those near the center.

This unexpected behavior suggests the presence of an invisible form of matter known as Dark Matter.

Current estimates indicate that:
 
Ordinary Matter ≈ 5%
Dark Matter ≈ 27%
Dark Energy ≈ 68%

This means that nearly 95% of the universe consists of components we cannot directly see.

Topics Covered
Galaxy Rotation Curves
Dark Matter Halo
Galaxy Mass Estimation
Gravitational Lensing
Dark Matter Visualization
Learning Objectives

After completing this chapter you will understand:

Why galaxies rotate the way they do
Evidence supporting Dark Matter
Rotation curves
Dark Matter halos
Gravitational lensing
Modern cosmology basics
